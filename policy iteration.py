...
def randomPolicyGenerate(stateMap):
    policy = []
...
def policyIteration(stateMap, PTFArray, rewardMap):
    global policy
    ident = np.eye(len(stateMap))
    piK = randomPolicyGenerate(stateMap)
    # with open('file.txt', 'w') as f:
    #     for x in range(len(piK)):
    #         print(piK[x], file=f)
    # print(piK)
    # print(len(piK))
    pPi = np.zeros((len(stateMap), len(stateMap)))
    rPi = np.zeros(len(stateMap))
    ...
    for cs in range(len(stateMap)):
        ...
        board[currAction[0]][currAction[1]] = 1
        zPos = zeroesPos(board)
        for o in zPos:
            board[o[0]][o[1]] = 2
            ns = stateMap.index(boardToDec(board))
            currValue += rewardMap[cs][boardToDec(
                currAction, True)][ns]*PTFArray[cs][boardToDec(currAction, True)][ns]
            board[o[0]][o[1]] = 0
        board[currAction[0]][currAction[1]] = 0
        rPi[cs] = currValue

    for cs in range(len(stateMap)):
        for ns in range(len(stateMap)):
            pPi[cs][ns] = PTFArray[cs][boardToDec(piK[cs], True)][ns]

    valueFunT = LA.inv(np.subtract(
        ident, np.multiply(0.3, pPi))).dot(rPi)

    while(True):
        # print("in")
        piKPlusOne = piK
        valueFunTPlusOne = valueFunT
        # print(valueFunTPlusOne.shape)
        for cs in range(len(stateMap)):
            board = decToBoard(stateMap[cs])
            maxAction = [-1, -1]
            ...
            for x in zPos:
                ...
                for o in zPosCopy:
                    ...
                    ns = stateMap.index(boardToDec(board))
                    currValue += rewardMap[cs][boardToDec(
                        np.array(x), True)][ns]*PTFArray[cs][boardToDec(np.array(x), True)][ns]
                    board[o[0]][o[1]] = 0
                board[x[0]][x[1]] = 0
                currValue += 0.3 * \
                    (np.dot(PTFArray[cs][boardToDec(
                        np.array(x), True)], valueFunT))
                if(currValue > maxValue):
                    maxValue = currValue
                    maxAction = x
            valueFunTPlusOne[cs] = maxValue
            piKPlusOne[cs] = maxAction

        # with open('file1.txt', 'w') as f:
        #     for x in range(len(piKPlusOne)):
        #         print(piKPlusOne[x], file=f)

        pPiPlusOne = np.zeros((len(stateMap), len(stateMap)))
        rPiPlusOne = np.zeros(len(stateMap))

        for cs in range(len(stateMap)):
            # print("cs ", cs)
            currValue = 0
            currAction = piKPlusOne[cs]
            board = decToBoard(stateMap[cs])
            # board = boardCopy[:]
            board[currAction[0]][currAction[1]] = 1
            zPos = zeroesPos(board)
            for o in zPos:
                ...
                board[o[0]][o[1]] = 0
            board[currAction[0]][currAction[1]] = 0
            rPiPlusOne[cs] = currValue

        for cs in range(len(stateMap)):
            for ns in range(len(stateMap)):
                pPiPlusOne[cs][ns] = PTFArray[cs][boardToDec(
                    piKPlusOne[cs], True)][ns]

        valueFunTPlusOne = LA.inv(np.subtract(
            ident, np.multiply(0.3, pPiPlusOne))).dot(rPiPlusOne)

        if(LA.norm(valueFunTPlusOne) - LA.norm(valueFunT) < 0.0001):
            policy = piKPlusOne
            break
        # valueFunT = valueFunTPlusOne
    return policy
...
policyIteration(stateMap, PTFArray, rewardMap)
