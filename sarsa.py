...
def actionPossibleEpsilon(stateMap, qTable, cs):
    epsilon = 0.3
    actionPossible = []
    ...
    for i in range(len(zPos)):
        if(qTable[cs][boardToDec(np.array(zPos[i]), True)] > maxVal):
            maxVal = qTable[cs][boardToDec(np.array(zPos[i]), True)]
            actionOne = zPos[i]

    actionPossible.append(actionOne)

    actionIdx = np.random.choice(range(len(zPos)), 1)[0]
    actionPossible.append(zPos[actionIdx])
    action = actionPossible[np.random.choice(
        range(2), 1, p=[1-epsilon, epsilon])[0]]
    action = boardToDec(np.array(action), True)
    return action


def sarsa(qTable, stateMap, actionStates, rewardMap, PTFArray):
    global policy
    cs = 0
    action = actionPossibleEpsilon(stateMap, qTable, cs)
    # print(action)
    for _ in range(300000):
        ns = np.random.choice(range(len(stateMap)), 1,
                              p=PTFArray[cs][action])[0]
        actionDash = actionPossibleEpsilon(stateMap, qTable, ns)
        qTable[cs][action] = qTable[cs][action] + 0.1 * \
            (rewardMap[cs][action][ns] + 0.3*qTable[ns]
             [actionDash] - qTable[cs][action])
        cs = ns
        action = actionDash
...
sarsa(qTable, stateMap, actionStates, rewardMap, PTFArray)
