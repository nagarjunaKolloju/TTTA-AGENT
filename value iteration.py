...
def valueIteration(stateMap, PTFArray, rewardMap):
    valueFunT = np.zeros(len(stateMap))
    global policy

    while(True):
        policy = []
        valueFunTPlusOne = np.zeros(len(stateMap))
        for cs in range(len(stateMap)):
            board = decToBoard(stateMap[cs])
            maxAction = [-1, -1]
            ...
            policy.append(np.array(maxAction))
        if(LA.norm(valueFunTPlusOne) - LA.norm(valueFunT) < 0.00001):
            break
...
valueIteration(stateMap, PTFArray, rewardMap)
...
print("TIE PERCENTAGE: ", (ties/noOfGames)*100)
